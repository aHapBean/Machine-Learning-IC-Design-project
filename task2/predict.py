...
class Predict(object):
    def __init__(self, now=True, future=True):
        self.now = now
        self.future = future

    def __call__(self, AIG):
        if self.now:
        ...
        for itm in dataloader:
            graph_data = itm.cuda()
            cur_aig_score = model_predict_now(graph_data).item() if self.now else 0.0
            future_aig_score = model_predict_future(graph_data).item() if self.future else 0.0
